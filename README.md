...
Installed updates download the best matching manifest asset, verify SHA256, stage it, stop AgentHero, apply it, restart, check `/api/health`, and roll back if startup fails.

Manifest assets can be:

- `patch`: a small platform-neutral app patch. Use this for normal UI/server/shared code changes where dependencies and install behavior did not change.
...
Patch assets are only selected when `fromVersion` matches the installed version. If no compatible patch exists, installed updates fall back to a matching full asset.
...
Before creating a release bundle, patch, or installer, increment the package build/patch number for routine builds, for example `0.1.0` to `0.1.1`. For significant updates, decide whether the minor or major version should be bumped before building.

Create a platform-neutral app patch from the currently published manifest version:
...
Both commands write a patch ZIP and a `patch-asset.json` snippet into `artifacts/`. Copy the patch ZIP to `installer/updates/<tag>/`, add the asset snippet to `installer/manifest.json`, commit, and push. Ask whether a change should ship as a patch before packaging it.
...
Do not rebuild the installer for every app change. Rebuild the installer only when the installer payload, install scripts, bootstrap behavior, or checked-in downloadable setup file needs to change. For normal app updates, publish a patch or full release asset and update `installer/manifest.json`.
